...
void loop()
{
  act_time = micros(); // act_time is defined as the number of microseconds since the Arduino board began running the current program
  difTime = act_time - time_ant;  // time difference is calculated as the difference between the actual time and time_ant intialized as 0 at the beginning 
  ...
  if (difTime >= 1000) {
    ...
    /***** The measured ADC inputs of the sensor and reference point are translated into voltage values using the formula x*5.0/1023 *****/
